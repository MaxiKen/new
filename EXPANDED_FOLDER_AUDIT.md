...
| 2026-09-12 | `007.md` 7:1 – 7:206 | **cross-quotations re-sourced to `translation/`** — every quotation of *another* chapter inside the commentary bodies now carries the repository wording verbatim. A dedicated read-only verifier extracted **1,068 quoted spans carrying a Qur'anic citation**, reduced them to word sequences (quote marks, brackets, ˹ ˺, dashes, punctuation stripped) and tested each against the cited verse(s) in `translation/`: baseline **264 DRIFT / 758 EXACT / 4 ELLIPSIS / 42 VICINITY**. Repairs, in order: (1) an automated snap-to-repository fixer replaced **106 spans** with the corresponding `translation/` wording (sentence-edge widening, edge-punctuation and quote-balance preserving, negation and weak-edge guards); (2) **54** hand-written replacements for spans the fixer refused or mis-scoped — among them §7:8's 18:49 / 6:61 / 43:80 and §7:32's 32:17 + 50:35; (3) **15** ellipsis-reconstruction repairs; (4) **14** qualified citations ("cf.", "vicinity", "context") whose quoted wording was still the drafter's own — 2:170 (§7:28), 8:50 (§7:37), 18:28 (§7:49), 26:109 (§7:66), 20:135 (§7:71), 4:29 (§7:85), 46:35 (§7:87), 76:30 (§7:89), 27:52 (§7:92), 35:40 (§7:99), 12:103 + 13:1 (§7:102), 7:43 (§7:104), 21:71 (§7:137), 7:69 (§7:140) — each put on the repository wording with its hedge dropped; (5) **12** quotes the snapping had truncated mid-phrase (43:80 "…messenger-angels are", 7:26 "This is one of", 23:118 "…the best of", 6:93 "…revealed to", 22:78 "…hardship in the", 25:4 "…which he", 6:116 "They follow nothing but", 4:100 "…their homes and", 5:66 "Some among them", 16:89 "…explanation of", 6:131 "Is because…", 17:51 "Will bring us back…") completed to the repository's own clause boundary. Three misattributions were resolved on the way: §7:4's quote belongs to 15:65 / 15:73 / 54:38, §7:32's to 32:17 + 50:35 (the ḥadīth-qudsī framing dropped), and §7:29's "I turned my face" is retained as a **marked literal gloss** with its second quote aligned to 3:20. Final verifier state: **1,071 spans — 1,026 EXACT / 0 DRIFT / 6 ELLIPSIS / 39 VICINITY**; the 6 ellipsis spans are verbatim fragments joined by an explicit `…`, and of the 39 VICINITY spans all are verbatim except §7:29's marked gloss. 217 lines changed, 293 quote spans touched. Depth preserved: `census.py --sura 7` reports **206 sections**, 197,198 → **197,687 words** (min 817, median 954, max 1,214; 0 sections below 260/400 words). Gates: `validate.py` **114/114 PASSED**; `normalize.py` idempotent (0 files written corpus-wide, `001.md` untouched); `test_skeleton.py` all checks passed; `check_scaffolding.py` 0 hits; `check_offtopic.py` 0 hits in `007.md`; `check_translations.py` 0 flagged in `007.md` (the 7 pre-existing flags in 010/050/068/094 unchanged); `align_own_verse.py report 7` unchanged at its 22 hand-reviewed accepted exceptions; `check_cites.py`'s residual LOW rows in `007.md` are matcher artifacts (a Ṣaḥīḥ Muslim 2175 report, and 33:36 / 32:16 / 39:53 quotes the tool pairs against adjacent citations). Ḥadīth quotations and explicitly qualified references remain out of scope. Scope: `007.md` only. |
| next | — | `007.md` is fully re-sourced: its verse translations, its inline own-verse quotations **and** its cross-quotations of other chapters all read verbatim from `translation/` (0 drift across 1,071 cited spans); remaining audit items are the priority list in §9 — the same three-stage re-sourcing has not been applied to the other 113 `expanded/*.md` files, which is a folder-level decision, not a `007.md` defect |
